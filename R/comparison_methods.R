...
MI_SVM <- function(data, cost, kernel = 'radial', max.step = 500, type = 'C-classification'){
    ...
    for( i in 1:n_bag){
        ...
        if(bag_i_label == 0 ){ ## this indicates a negative bag
    ...
    names(weights) <- c('1', '0')
    ## iterate between updating the model and selecting the most positive bag from an instance.
...
predict.MI_SVM <- function(object, ...){
    ...
    if(! is.null(true_bag_info)){
        ...
        AUC <- pROC::auc(ROC)
        return(list("instance_level_prediction" = data_instance, "bag_level_prediction" = data_bag, "ROC" = ROC, "AUC" = AUC))
...
##' @export 
##' @author Yifei Liu
smm_bag <- function(data, kernel_mild = "rbf", cost = 1, class.weights, sigma = 0.05, ...){
    ## Let's assume here that the data structure looks something like
    ## bag_label | bag_name | instance_name |  feature_1 | ...
    ## bag_label should be one of 0 and 1, where 0 is negative bags and 1 is positive bags
    ## we need to prepare the data into the following format so that the smm() function can be called.
    ...
    if(is.matrix(kernel_mild)){
        if(is.null(list(...)$y)) stop( " 'y' should be supplied if 'kernel_mild' is a matrix")
        res <- SMM(df = NULL, kernel_mild = kernel_mild, cost = cost, class.weights = class.weights, sigma = sigma, y = list(...)$y)
    }else{
        res <- SMM(df = data, kernel_mild = kernel_mild, cost = cost, class.weights = class.weights, sigma = sigma, y = NULL)
    }

    return(
        validate_smmBag(
            new_smmBag(
                list("object" = res, "traindata" = original_data)
            )
        )
    )
}
...
predict.smmBag <- function(object, ...){
    ## newdata should have | bag_name | instance_name | feature 1 | ..., with an optional bag_label as the first column.

    args <- list(...)
    newdata = args$newdata
    if(is.null(newdata)){
        newdata <- object$traindata
        if(!is.null(newdata$bag_label)) true_bag_info <- unique(newdata[, c("bag_label", "bag_name")])
        else true_bag_info <- NULL

    }else{
        if(!is.null(args$true_bag_info)) true_bag_info <- args$true_bag_info
        else if(!is.null(newdata$bag_label)) true_bag_info <- unique(newdata[, c("bag_label", "bag_name")])
        else true_bag_info <- NULL
    }
    ...
    results <- cbind(unique(newdata[, c("bag_name", "instance_name")]), instance_score_pred, (instance_score_pred > 0))
    colnames(results)[3:4] <- c("instance_score_pred", "instance_label_pred")
    results_bag <- results %>% dplyr::group_by(bag_name) %>%
        dplyr::summarise(bag_score_pred = max(instance_score_pred),
                         bag_label_pred = bag_score_pred > 0
                         )
    if(!is.null(true_bag_info)){
        results_bag <- dplyr::inner_join(results_bag, true_bag_info, by = "bag_name")
        ROC <- pROC::roc(response = results_bag$bag_label, predictor = results_bag$bag_score_pred)
        AUC <- pROC::auc(ROC)
        return(list("instance_level_prediction" = results,
                    "bag_level_prediction" = results_bag,
                    "AUC" = AUC,
                    "ROC" = ROC
                    )
               )
...
##' @export 
##' @author Yifei Liu
cv_smm_bag <- function(data, n_fold, fold_id, cost_seq, kernel_mild = "rbf", class.weights, sigma = 0.05){

    bag_info <- unique(data[, c("bag_label", "bag_name")])
    if(missing(fold_id)){
        if(missing(n_fold)) n_fold = 5
        ...
        temp_data <- data.frame("bag_name" = unique(data$bag_name), "bag_id" = bag_id, stringsAsFactors = FALSE) %>% right_join(data %>% select(bag_name), by = "bag_name")
        fold_id <- temp_data$bag_id
    }else{
        n_fold <- max(fold_id)
        if( !is.null( setdiff(fold_id, 1:n_fold) )) stop("The argument fold_id has some 'holes'!") 
    }
    ...
    for(C in 1:length(cost_seq)){
        ...
        for(i in 1:n_fold){
            ...
            mdl <- smm_bag(data = data_train,
                           cost = cost_seq[C],
                           kernel_mild = kernel_mild,
...
##' @export 
##' @author Yifei Liu
build_instance_feature <- function(data, qtls = seq(0.05, 0.95, length.out = 10), mean = TRUE, sd = TRUE){
    ## Let's assume here that `data` is a MilData object which looks something like
    ## bag_label | bag_name | instance_name |  feature_1 | ...
    ## bag_label should be one of 0 and 1, where 0 is negative bags and 1 is positive bags
    ## we need to prepare the data into the following format so that the MI_SVM function can be called.
    ## bag_label | bag_name | instance_name |  feature_1 | ...
    ...
    for (i in 1:length(instance_name)){
        ...
        qtls_i <- as.vector(apply(data_i, 2, function(x) stats::quantile(x, probs = qtls)))
        if(mean){
    ...
    col_name <- as.vector(sapply(colnames(data[, -(1:3)]), function(x) paste0(x, "_", qtls)))
    if(mean){
...
##' @export 
##' @author Yifei Liu
mil_with_feature <- function(data, kernel = "radial", cost = 1, class.weights = NULL, sigma = 0.05, type = 'C-classification', qtls = seq(0.05, 0.95, length.out = 10), max.step = 500){
    df <- build_instance_feature(data, qtls)
    mdl <- MI_SVM(data = df, cost = cost, kernel = kernel, max.step = max.step, type = type)
    return(mdl)
}
...
##' @export 
##' @author Yifei Liu
cv_MI_SVM <- function(data, n_fold, fold_id, cost_seq, kernel = 'radial', max.step = 500, type = 'C-classification'){
    bag_info <- unique(data[, c("bag_label", "bag_name")])
    if(missing(fold_id)){
        if(missing(n_fold)) n_fold = 5
    ...
    for(C in 1:length(cost_seq)){
        ...
        for(i in 1:n_fold){
            data_train <- data[fold_id != i, ]
            data_valid <- data[fold_id == i, ]
            mdl <- MI_SVM(data = data_train,
                          cost = cost_seq[C],
                          kernel = kernel,
                          max.step = max.step,
                          type = type
                          )
            predictions_i <- predict(object = mdl, newdata = data_valid, true_bag_info = unique(data_valid[, 1:2]))
            temp_auc <- temp_auc + predictions_i$AUC
        }
        AUCs[C] <- temp_auc / n_fold
    }
