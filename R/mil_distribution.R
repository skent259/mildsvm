...
##' 
##' @author Yifei Liu
kernel_mil <- function(kernel_full, data_info, max.step, cost, weights, sigma, yy, useful_inst_idx){
    ...
    step <- 1
    while(step < max.step){
        svm_model <- SMM(df = NULL, kernel_mild = kernel_full[useful_inst_idx, useful_inst_idx], cost = cost, class.weights = weights, sigma = sigma, y = yy_inst)
        ## need to get the predicted score for each instance of the original data. This should be carefully handled
        ...
        for(i in 1:n_bag){
            ...
            if( data_i$bag_label[1] == 0 ){ ## negative bag
                useful_inst_idx <- c(useful_inst_idx, (last_inst_idx + 1):(last_inst_idx + n_inst_i))
            }else{
                id_max <- which.max(pred_all_score[ (last_inst_idx + 1): (last_inst_idx + n_inst_i)])
                selection[pos_idx] <- id_max
                useful_inst_idx <- c(useful_inst_idx, last_inst_idx + id_max)
                pos_idx <- pos_idx + 1
            }
    ...
    return(list("svm_model" = svm_model, "useful_inst_idx" = useful_inst_idx, "step" = step, "selection" = selection, "difference" = difference, "repeat_selection" = repeat_selection, "n_bag" = n_bag))
}
...
##' 
##' @author Yifei Liu
mil_distribution_original_no_pre_kernel <- function(data, cost, max.step = 500, sigma = 0.05){
    ## data should be of a MilData object.
    ...
    ## Calculate the full kernel matrix, kernel_full is a length(instance_name) by length(instance_name) matrix.
    ...
    for( i in 1:n_bag){ ## loop over all unique bags.
        ...
        if( data_i$bag_label[1] == 0 ){ #if all instances in this bag has a negative label, then this is a negative bag
            

            useful_inst_idx <- c(useful_inst_idx, (last_inst_idx + 1):(last_inst_idx + n_inst_i)) ## add instances in this bag to instance index
        ...
        }else if( data_i$bag_label[1] == 1 ){ ## if the bag is a positive bag
            ...
            stop(paste("Bag labeling is inconsitent for ", i, "-th bag, ", unique_bag_name[i]))
        }
    }
    ...
    temp_res <- kernel_mil(kernel_full, data_info, max.step, cost, weights, sigma, yy, useful_inst_idx)
    
    sample_df <- data[data$instance_name %in% instance_name[temp_res$useful_inst_idx], -c(1, 2)]
    res <- new_mild(list("model" = temp_res$svm_model, "total_step" = temp_res$step, "representative_inst" = cbind(positive_bag_name, temp_res$selection), "traindata" = sample_df))
    return(res)
}
...
mil_distribution <- function(data, cost, max.step = 500, sigma = 0.05, ...){
    ...
    sample_df <- data[data$instance_name %in% instance_name[temp_res$useful_inst_idx], -c(1, 2)]
    res <- new_mild(list("model" = temp_res$svm_model, "total_step" = temp_res$step, "representative_inst" = cbind(positive_bag_name, temp_res$selection), "traindata" = sample_df, "useful_inst_idx" = temp_res$useful_inst_idx))
    return(res)
}
...
##' @author Yifei Liu
predict.mild <- function(object, ...){# newdata){
    args <- list(...)
    newdata = args$newdata
    ##auc = list(...)$newdata
    ##if(!is.null(auc) & auc & is.null(newdata$bag_label)) stop("newdata$bag_label should exist for AUC!")
    
    ## newdata should have form
    ...
    ##label_pred <- predict(object = mdl$object, newx = newdata[, -(1:3)])
    ## instance_label has the same length as the number of instances

    if(is.null(newdata$bag_label)){
        instance_score <- predict(object = object$model, newdata = newdata %>% select(-bag_name), traindata = object$traindata, kernel_mild = object$kernel_mild)

        labels <- as.data.frame(cbind(newdata %>% dplyr::group_by(instance_name) %>% dplyr::summarise(bag_name = bag_name[1]), instance_score))

        bag_label <- labels %>% dplyr::group_by(bag_name) %>% dplyr::summarise(bag_label_pred = any(instance_score > 0))
        final_pred <- newdata %>% dplyr::select(bag_name, instance_name) %>% dplyr::left_join(bag_label, by = "bag_name") %>% dplyr::left_join(labels %>% dplyr::mutate(instance_label = instance_score > 0) %>% dplyr::select(instance_name, instance_score), by = "instance_name")
        return(final_pred)
    }

    if(is.null( args$GramMatrix)){
        instance_score <- predict(object = object$model, newdata = newdata %>% select(-bag_name, -bag_label), traindata = object$traindata, kernel_mild = object$kernel_mild)
    }else{
        instance_score <- predict(object = object$model,  kernel_mild = args$GramMatrix[object$useful_inst_idx, ])
    }
    
    labels <- as.data.frame(cbind(unique(newdata[, c("bag_name", "instance_name", "bag_label")]), instance_score))
    
    labels_bag_level <- labels %>% dplyr::group_by(bag_name) %>% dplyr::summarise(bag_label_pred = any(instance_score > 0),
                                                                                  bag_score =  max(instance_score),
                                                                                  bag_label = bag_label[1])
    final_pred <- newdata %>% dplyr::select(bag_name, instance_name) %>% dplyr::left_join(labels_bag_level, by = "bag_name") %>% dplyr::left_join(labels %>% dplyr::mutate(instance_label_pred = instance_score > 0) %>% dplyr::select(instance_name, instance_score, instance_label_pred), by = "instance_name")
...
##' @export 
##' @author Yifei Liu
cv_mild <- function(data,  n_fold, fold_id,  cost_seq = 2^(-2:2), max.step = 300, sigma = 0.05, ...){
    ...
    if(missing(fold_id)){
        ...
        temp_data <- data.frame("bag_name" = unique(data$bag_name), "bag_id" = bag_id, stringsAsFactors = FALSE) %>% dplyr::right_join(unique(data %>% dplyr::select(bag_name, instance_name)), by = "bag_name") 
    ...
    for(C in 1:length(cost_seq)){
        ...
        for(i in 1:n_fold){
            ...
            train_useful_inst_idx <- match(base::intersect(train_instance_id, useful_inst_idx) , train_instance_id)
            ...
            predictions_i <- predict(object = temp_mdl$svm_model, kernel_mild = kernel_full[train_instance_id[temp_mdl$useful_inst_idx], valid_instance_id])

            data_valid_info <- data_info[valid_instance_id, ]
            data_valid_info$instance_score_pred <- predictions_i
            bag_score_pred <- (data_valid_info %>% dplyr::group_by(bag_name) %>% dplyr::summarise(bag_score_pred = max(instance_score_pred)))$bag_score_pred
            temp_auc <- temp_auc + pROC::auc(pROC::roc(response = true_valid_bag_label, predictor = bag_score_pred))
        }
        AUCs[C] <- temp_auc / n_fold
    }

    bestC <- cost_seq[which.max(AUCs)]
    temp_res <- kernel_mil(kernel_full = kernel_full,
                          data_info = data_info,
                          max.step = max.step,
                          cost = bestC,
                          weights = weights,
    ...
    sample_df <- data[data$instance_name %in% instance_name[temp_res$useful_inst_idx], -c(1, 2)]
    res <- new_mild(list("model" = temp_res$svm_model, "total_step" = temp_res$step, "representative_inst" = cbind(positive_bag_name, temp_res$selection), "traindata" = sample_df, "useful_inst_idx" = useful_inst_idx))
...
##' 
##' @author Yifei Liu
cv_mild_old <- function(data,  n_fold, fold_id,  cost_seq = 2^(-2:2), max.step = 300, sigma = 0.05){
