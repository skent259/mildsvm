if(getRversion() >= "2.15.1")  utils::globalVariables(c("bag_name", "bag_label", "instance_label", "instance_name", "label", "bag_score_pred", "instance_label_pred", "instance_score_pred"), add = FALSE)
...
validate_MilData <- function(x){
    ...
    consist <- unclass(value %>% dplyr::group_by(bag_name) %>% dplyr::summarise(consist = length(unique(bag_label)) == 1))$consist
    if( any(consist != TRUE) ){
        stop( paste("There is inconsistency bag labeling at bag ", unique(value$bag_name)[which(consist != TRUE)]))
        call. = FALSE
    }

    ## verify that the bag_label is determined by whether there is at least a positive label in instances of a bag.
    
    x_instance_label <- attr(x, "instance_label")
    if( ! is.null(x_instance_label) ){
        value$instance_label = x_instance_label
        consist_inst <- unclass(value %>% dplyr::group_by(bag_name) %>% dplyr::summarise(consist_inst = any(as.logical(instance_label)) == unique(bag_label)))$consist_inst
        if( any(consist_inst != TRUE) ){
            stop(paste("There is inconsistency in instance-bag labelling in bag ", unique(value$bag_name)[which(consist_inst != TRUE)]))
            call. = FALSE
        }
    }
    x
...
##'
##' Helper function to generate MilData object. Changes the first three columns to the required column names ('bag_label', 'bag_name', 'instance_name'). If the original `bag_label` is a factor, will use the first level as negative label and code it as numeric '0' and use the second level as positive label and code it as numeric '1'. Levels >= 3 not supported and will raise an error.
##'
...
MilData <- function(x = data.frame()){
    ...
    if(is.factor(x$bag_label)){
        ...
        if(length(levels) == 1) stop("There is only one level for the bag label!")
        x$bag_label = unclass(x$bag_label) - 1
        message( paste("Using the first level of bag label, ", levels[1], ", as negative label and the second level, ", levels[2], ", as positive label"))
    }
...
GenerateMilData <- function(positive_dist = c("mvt", "mvnormal"),
    ...
    ## remainder follows a distr with mean 0 and scale matrix identity. (Hence different cov matrix for t and normal)
    ## the positive_cov and negative_cov's are exactly the cov for the positive or negative distributions, whether or not they are t or normal.
    ## should pass "positive_degree", "negative_degree" or "remainder_degree" in "..." if any of these distributions is specified as "mvt"
    ## sanity check
    ...
    for(i in 1:nbag){
        ...
        for(j in 1:ninst){
            if(is.null(ins_labels)) instance_label <- stats::rbinom(1, 1, positive_prob) else instance_label <- ins_labels[j]
            
            if(instance_label == 1){
                positive_features <- switch(positive_dist,
                                            mvt = if(is.null(args$positive_degree)) stop("Needs to supply 'positive_degree' in '...' when using mvt for positive distribution!") else mvtnorm::rmvt(n = nsample,
                                                                                                                                                                                      sigma = positive_cov / (args$positive_degree / (args$positive_degree - 2) ), ## hence the covariance matrix of the resulting distribution will be positive_cov.
                                                                                                                                                                                      df = args$positive_degree, delta = positive_mean, type = "shifted"
                                                                                                                                                                                      ),
                                            mvnormal = mvtnorm::rmvnorm(n = nsample, mean = positive_mean, sigma = positive_cov ))
                remainder_p <- ncov - length(nimp_pos)
                data_ij <- matrix(NA, nsample, ncov)
                data_ij[, nimp_pos] <- positive_features
                if(remainder_p > 0){
                    remainder_features <- switch(remainder_dist,
                                                 mvt = if(is.null(args$remainder_degree)) stop("Needs to supply 'remainder_degree' in '...' when using mvt for remainder distribution!") else mvtnorm::rmvt(n = nsample, sigma = diag(1, remainder_p) / (args$remainder_degree / (args$remainder_degree - 2)), df = args$remainder_degree, delta = rep(0, remainder_p), type = "shifted"),
                                                 mvnormal = mvtnorm::rmvnorm(n = nsample, mean = rep(0, remainder_p), sigma = diag(1, remainder_p))
                                                 )
                ...
                negative_features <- switch(negative_dist,
                                            mvt = if(is.null(args$negative_degree)) stop("Needs to supply 'negative_degree' in '...' when using mvt for negative distribution!") else mvtnorm::rmvt(n = nsample, sigma = negative_cov / (args$negative_degree / (args$negative_degree - 2)), df = args$negative_degree, delta = negative_mean, type = "shifted"),
                                            mvnormal = mvtnorm::rmvnorm(n = nsample, mean = negative_mean, sigma = negative_cov )
                                            )
            ...
            data_ij <- cbind(rep(paste0("bag", i, "inst", j), nsample), as.data.frame( data_ij ), rep(instance_label, nsample))
            data_i <- rbind(data_i, data_ij)
        }
        bag_name <- rep(paste0("bag", i), nrow(data_i))
        if(is.null(bag_label)) bag_label <- rep(any(data_i[, ncov+2] == 1), nrow(data_i))
        data <- rbind(data, cbind(bag_label, bag_name, data_i))
    }
    colnames(data) = c("bag_label", "bag_name", "instance_name", paste0("X", 1:ncov), "instance_label")
    data <- data %>% dplyr::mutate(bag_label = as.numeric( bag_label),
                                   bag_name = as.character( bag_name ),
                                   instance_name = as.character( instance_name)
                                   )
    data <- MilData(data)
    return(data)
}
