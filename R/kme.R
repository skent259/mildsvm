...
kme.default <- function(df, df2 = NULL, sigma = 0.05) {
    ## this function calculates the kernel mean embedding for a given
    ...
    if (is.null(df2)) {
        if (is.null(df$instance_name)) {
            ...
            for (i in 1:n) {
                ...
                for (j in 1:i) {
                  ...
                  n_j <- nrow(Z_j)
                  dis_ij <- apply(X_i, 1, function(x) apply(Z_j, 1, function(z) sum((x - 
                    z)^2)))  ## this line, together with next line, will make the calculation much faster.
        ...
        if (is.null(df$instance_name) | is.null(df2$instance_name)) {
            ...
            for (i in 1:n) {
                X_i <- df[df$instance_name == inst_name_set[i], ]
                X_i$instance_name = NULL
                # X_i <- df %>% dplyr::filter(instance_name == inst_name_set[i]) %>%
                ...
                for (j in 1:n2) {
                  ...
                  Z_j$instance_name = NULL
                  # Z_j <- df2 %>% dplyr::filter(instance_name == inst_name_set2[j])
                  # %>% dplyr::select(-instance_name)
                  n_j <- nrow(Z_j)
                  dis_ij <- apply(X_i, 1, function(x) apply(Z_j, 1, function(z) sum((x - 
                    z)^2)))
