...
##'
##' Function to construct mild object. `x` should be a list with components
##'   `model` (an smm object), `total_step` (a integer), `representative_inst`
##'   (a matrix of two columns), `traindata` (a data.frame with the first
##'   column being 'instance_name')
##' @param x A list
...
#' Validate a "mild" object
#' @export
#' @author Sean Kent
validate_mild <- function(x) {
    message("No validations currently in place for object of class 'mild'.")
    x
}

#' Construct a "mildsvm" object
#' @export
#' @author Sean Kent
new_mildsvm <- function(x = list(), method = "mip") {
    stopifnot(is.list(x))
...
##' @param ... should pass 'newdata', the new dataset to be predict on,
##'   optionally 'GramMatrix' to accelerate the speed.
##' @return A data.frame with the same number of rows as in newdata and with
##'   four columns, 'bag_name', 'instance_name', 'bag_label_pred', 'instance_score'
##' @examples
##' MilData1 <- GenerateMilData(positive_dist = 'mvt',
##'                             negative_dist = 'mvnormal',
##'                             remainder_dist = 'mvnormal',
...
cv_mild <- function(data, n_fold, fold_id, cost_seq = 2^(-2:2), max.step = 300,
    ...
    instance_selection <- initialize_instance_selection(data)
    useful_inst_idx = instance_selection[["useful_inst_idx"]]
    yy_inst = instance_selection[["yy"]]

    num_neg_inst <- length(useful_inst_idx) - length(positive_bag_name)
    weights <- c(length(positive_bag_name)/num_neg_inst, 1)  ## this is less affected by the total number of bags.
    names(weights) <- c("0", "1")
    ...
    for (C in 1:length(cost_seq)) {
        ...
        for (i in 1:n_fold) {
            ind <- which(fold_id != i)
            valid_ind <- which(fold_id == i)
            train_useful_inst_idx <- match(base::intersect(ind,
                useful_inst_idx), ind)
            temp_mdl <- kernel_mil(kernel_full = kernel_full[ind, ind],
                                   data_info = data_info[ind, ],
                                   max.step = max.step,
                                   cost = cost_seq[C],
                                   weights = weights,
                                   sigma = sigma,
            ...
            true_valid_bag_label <- unique(data_info[valid_ind, 1:2])$bag_label
            predictions_i <- predict(object = temp_mdl$svm_model,
                                     kernel_mild = kernel_full[ind[temp_mdl$useful_inst_idx], valid_ind])

            data_valid_info <- data_info[valid_ind, ]
...
##' )
##' ### This takes about 20 minutes.
##' ### foo <- cv_mildsvm(MilData1, n_fold = 2)
##' ###
##' @export
##' @author Sean Kent
cv_mildsvm <- function(data, n_fold, fold_id, cost_seq = 2^(-2:2),
    method = "mip", time_limit = 150, kfm_fun = "kfm_nystrom", ...) {
    ...
    if (method == "mip") {
        ...
        res <- cv_misvm_mip_gurobi(y, b, X, fold_id, cost_seq, rescale = FALSE, time_limit = time_limit)
    } else {
        stop("Please specify method = 'mip' for now. ")
    }

    res$BestMdl <- new_mildsvm(res$BestMdl, method = method)
    res$BestMdl$kfm_fit <- kfm_fit
...
#' @export
#' @author Sean Kent
predict.mildsvm <- function(object, newX, type = c("prediction", "score")) {
    type <- type[1]
    type <- match.arg(type, c("prediction", "score"))
    # need to convert the underlying data to the same feature space as before
    if (attr(object, "method") == "mip") {
        kmm <- build_kernel_mean_map(object$kfm_fit, newX)
        X_kmm <- subset(kmm, select = -c(bag_label, bag_name, instance_name))
        scores <- predict.misvm(object, X_kmm, type = "score")
    }

    if (type == "prediction") {
        pred <- predict.misvm(object, X_kmm, type = "prediction")
    } else if (type == "score") {
        pred <- scores
    }

    roc <- pROC::roc(response = classify_bags(kmm$bag_label, kmm$bag_name),
                     predictor = classify_bags(scores, kmm$bag_name))
    auc <- pROC::auc(roc)
    # TODO: pred comes out as a matrix, can make it into a vector...
    return(list(final_pred = pred,
                AUC = auc,
                ROC = roc))
}
...
#' Function to reorder the data by bag label, bag number, and then data
#'
